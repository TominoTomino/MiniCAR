...
void OLED_ShowHexNumber(u8 x, u8 y, u32 num, u8 len, u8 size)
{
    ...
    for(t = 0; t < len; t++)
    {
        ...
        if(temp < 10)
            ...
            OLED_ShowChar(x + (size / 2)*t, y, temp - 10 + 'A', size, 1);
    }
}

//显示十进制 带符号数
//x,y :起点坐标
//num：要显示的数字，范围：-2147483648~2147483648
//len :带小数点、负号数据个数
//size：字体大小
...
void OLED_ShowSignedNumber(u8 x, u8 y, float num, u8 len, u8 size)
{
    ...
    for(t = 0; t < len; t++)
    {
        ...
        OLED_ShowChar(x + (size / 2)*t, y, temp + '0', size, 1); //一位一位显示下去
